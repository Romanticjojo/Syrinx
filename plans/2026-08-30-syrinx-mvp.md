...
- [x] 步骤 1：`npm run build` 零错误（exit 0，607ms）；`npm run test` 全 PASS（17/17）；`npm run dev` 启动（5173 端口 242ms），HTTP 验证首页/manifest/曲谱/flute.glb 全 200，无头浏览器渲染曲库 DOM 正常（11 张卡片元素、无报错）；听感/视觉类项列入人工清单（见下）
- [x] 步骤 2：P0 清单逐项核对——P0-1~P0-8、P0-10 全部实现；P0-9 以 3D 长笛 Intro 实现（外部拼装视频素材预留，可跳过）；P2 音高检测基础已提前达成（YIN + 对比统计，超 MVP 计划范围）
- [x] 步骤 3：oxlint 通过（0 error 0 warning；顺手修复 PreviewPage effect 内 setState 告警）；无死代码/临时验证代码残留（grep 复查仅存文档化占位替换点）
- [x] 步骤 4：设计定稿文档已随任务 0 归档至 `docs/设计/`（按收尾约束只读，不再回改状态；工程侧以本计划勾选为准）
- [x] 步骤 5：最终 Commit「chore: MVP 收尾验证」

**任务 13 人工验收清单（需真机浏览器 + 麦克风，自动验证无法覆盖的听感/视觉项）：**
1. 入场动画可跳过、「下次不再播放」记忆生效
2. 曲库 → 预览：3D 长笛展示、曲谱静态渲染与缩放
3. 演奏：4 拍倒数 → 伴奏响起，光标与伴奏同步（听感 <50ms）、自动滚动、晨光背景随音量呼吸、谱面可读
4. 演奏中控件 3.2s 无操作自动隐藏、鼠标/键盘唤醒
5. 麦克风授权 → 结束后回放页：录音可播、音高图表与统计合理
6. 音量/变速/seek/回开头控制正常
