...
Web-deploy integration was also checked after merging: all 567 tests passed with its four-song environment, the production build passed and excluded the PersonalLibrary chunk. Computer Use confirmed the full-version notice, the four featured cards, Luv Letter's 100→80 BPM preparation, measure 5 selection and a fully visible 320×640 tempo dialog. The web branch's default development command uses web mode.
